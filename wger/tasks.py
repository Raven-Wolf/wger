...
@task(
    help={
        ...
        'process-static': 'Whether to process static files (install npm and process css). Default: True',
    }
)
...
def setup_django_environment(settings_path: str = None):
    ...
    if not os.path.isfile('manage.py'):
        print('Error: please run this script from the wger checkout folder')
        sys.exit(1)
...
def database_exists():
    ...
    try:
        ...
        print(f'Your settings file seems broken: {e}')
        sys.exit(0)
    else:
        return True
